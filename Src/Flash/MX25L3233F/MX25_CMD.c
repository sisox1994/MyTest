...
void CS_Low()
{
  HAL_GPIO_WritePin( SPI_GPIO , PIN_NSS , GPIO_PIN_RESET);
}

void CS_High()
{   
  HAL_GPIO_WritePin( SPI_GPIO , PIN_NSS ,GPIO_PIN_SET);
}
/*
* Function:       Flash_Init
* Arguments:      None
...
void SPI_GPIO_Setting(){
    ...
    GPIO_InitStruct.Alternate = GPIO_AF5_SPI2;                //Modify Need
    HAL_GPIO_Init(SPI_GPIO, &GPIO_InitStruct);
}

void SPI_NSS_Setting(){
...
void SendByte( uint8 byte_value, uint8 transfer_type )
{
  ...
  HAL_SPI_Transmit(&hspi2,test, 1, 5);
}
/*
* Function:       GetByte
...
uint8 GetByte( uint8 transfer_type )
{
  ...
  uint8 out;
  HAL_SPI_Receive(&hspi2, data_buf, 1, 5);
  out = data_buf[0];
  return out;
}
