...
void APP_background_Broadcast(){
    ...
    if(T5s_cmd39_Flag){
        ...
        if(ucSubSystemMode == C_App_IdleVal){
            ...
            if(Cloud_Run_Initial_Busy_Flag != 1){
                Btm_Task_Adder(C_39Val);
            }
            
        }else{
            
            switch(ucSubSystemMode){
  
              case C_App_SetupVal: 
              case C_App_WarmUpVal:
              case C_App_CoolDownVal:
              case C_App_PauseVal:
              case C_App_StopVal:
...
void CloudRun_Workout_Data_Broadcast(){
    ...
    if(T500ms_cmd39_Flag){
        ...
        if (ucSubSystemMode == C_App_RunningVal){  //&& (Program_Select == APP_Cloud_Run)){
            Btm_Task_Adder(C_39Val);
        }
                ...
                //for(unsigned char i =0; i < btm_Task_Cnt; i++){
                    ...
                    switch(btmTask_List[0]){
                     ...
                     //--------------雲跑 --Cloud Run    -------------------------
                      ...
                      //case  FTMS_HR_Connect:
                       // F_Btm_FTMS_B0_HR_Switch(1);
                        //break;
                        
                      //case  FTMS_HR_Disconnect:
                        //F_Btm_FTMS_B0_HR_Switch(0);
                        //break;
